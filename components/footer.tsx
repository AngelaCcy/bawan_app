...
const Footer = () => {
  return (
    <footer className="flex flex-col text-gray-500 text-lg mt-5 bg-navbar">
      <div className="flex flex-wrap justify-center max-sm:flex-col gap-10 px-6 py-10 sm:gap-20 mx-8">
          ...
          <div key={links.title} className="px-6">
            ...
            <div className="flex flex-col gap-3">
              {links.links.map((link) => (
                <Link key={link.title} href={link.url} className="md:text-sm">
                  {link.title}
                </Link>
              ))}
            </div>
      ...
      <div className="flex justify-center md:text-xs pb-4">
